...
function messageExists(el, attr) {
  return el.nextElementSibling && el.nextElementSibling.getAttribute(attr)
    ? true
    : false;
}
...
const validateForm = (function validateForm() {
  if ($$.contactForm) {
    let input, regex, invalidInputs, errorMessage, errorContainer;
    ...
    $$.contactForm.addEventListener("keyup", function (e) {
      ...
      if (input.hasAttribute("data-regex")) {
        ...
        if (regex.test(input.value)) {
          ...
          input.hasAttribute("data-valid")
            ? input.setAttribute("data-valid", "true")
            : null;

          // Check if input contains specified class, if so - remove it
          input.classList.contains("form-input-field--invalid")
            ? input.classList.remove("form-input-field--invalid")
            : null;
          ...
          input.classList.contains("form-input-field--invalid")
            ? null
            : input.classList.add("form-input-field--invalid");
    ...
    $$.contactForm.addEventListener("focusout", function (e) {
      ...
      if (!input.value && input.classList.contains("required")) {
        ...
        input.classList.contains("border-red-100")
          ? null
          : input.classList.add("border-red-100");

        // Show 'Required' pop-up on the top of the form
        $$.requiredPopUp.classList.contains("hidden")
          ? $$.requiredPopUp.classList.remove("hidden")
          : null;
      } else {
